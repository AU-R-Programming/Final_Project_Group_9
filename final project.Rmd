...
#This vignette demonstrates the functionality of the **FinalProjectGroup9** package, which provides tools for logistic regression using numerical optimization, bootstrapped confidence intervals, and classification performance metrics.
...
crop_data <- read.csv("~/Documents/GitHub/Final_Project_Group_9/binclass/crop.data.csv", header = TRUE, sep = ",")
head(crop_data)
...
#Copy code
# Prepare predictors (add intercept)

X <- as.matrix(cbind(1, crop_data[, c("predictor1", "predictor2")])) # Adjust columns as necessary
# Response variable
y <- as.numeric(crop_data$response)
...
```{r}
crop <- read.csv("~/Documents/GitHub/Final_Project_Group_9/binclass/crop.data.csv", header = TRUE, sep = ",")
head(crop)
